...
import { MioHook, MioHookPoint } from '../../../miogram/hooks/MioHook';
import MiogramModernPlayerModal from '../../../miogram/ui/MiogramModernPlayerModal';
...
const AudioPlayer = ({
  ...
  useEffect(() => {
    if (audio) {
      MioHook.notify(MioHookPoint.AUDIO_TRACK_CHANGED, {
        title: audio.title,
  ...
  return (
    ...
    >
      ...
      {audio && (
        ...
        >
          <span style="font-size: 1rem; font-weight: bold; color: var(--color-primary);">♪</span>
        </Button>
      )}
      ...
      {isLyricsModalOpen && audio && (
        <MiogramModernPlayerModal
          isOpen={isLyricsModalOpen}
          title={audio.title || ''}
          artist={audio.performer || ''}
          duration={audio.duration || mediaDuration || 0}
          currentTime={audioProxy ? audioProxy.currentTime : (playProgress ? playProgress * (audio.duration || mediaDuration || 0) : 0)}
          isPlaying={isPlaying}
          onClose={() => setIsLyricsModalOpen(false)}
          onSeek={(timeSec) => setCurrentTime?.(timeSec)}
