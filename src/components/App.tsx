import { useEffect, useLayoutEffect, useState, useCallback } from '../lib/teact/teact';
import { withGlobal } from '../global';
import MiogramNyaCurtain from '../miogram/ui/MiogramNyaCurtain';
import { MiogramVault, PinVerdict } from '../miogram/vault/MiogramVault';
...
const App = ({
  ...
  const [isMiogramLocked, setIsMiogramLocked] = useState(MiogramVault.isLocked());
  const handleMiogramUnlocked = useCallback((verdict: PinVerdict) => {
    setIsMiogramLocked(false);
  }, []);
  ...
  return (
    <UiLoader page={page} isMobile={isMobile}>
      ...
      <Notifications />
      <MiogramNyaCurtain isLocked={isMiogramLocked} onUnlocked={handleMiogramUnlocked} />
    </UiLoader>
  );
};
